...
def create_network(inp, channels_out, name):
    with tf.name_scope(name):
        ...
        layer = InstanceNormalization()(layer)
        layer2 = kl.Activation("relu")(layer)

        layer = kl.Conv2D(256, 3, padding="same", strides=2)(layer2)
        layer = InstanceNormalization()(layer)
        layer3 = kl.Activation("relu")(layer)

        # Transformer
        layer = ResidualBlock(256, nb_layers=2, kernel_size=3, normalization="instancenorm")(layer3)
        layer = ResidualBlock(256, nb_layers=2, kernel_size=3, normalization="instancenorm")(layer)
        layer = ResidualBlock(256, nb_layers=2, kernel_size=3, normalization="instancenorm")(layer)
        layer = ResidualBlock(256, nb_layers=2, kernel_size=3, normalization="instancenorm")(layer)
        layer = ResidualBlock(256, nb_layers=2, kernel_size=3, normalization="instancenorm")(layer)
        layer = ResidualBlock(256, nb_layers=2, kernel_size=3, normalization="instancenorm")(layer)
        ...
        # Decoder
        layer = kl.Concatenate()([layer, layer3])
        layer = kl.Conv2DTranspose(128, 3, padding="same", strides=2)(layer)
        layer = InstanceNormalization()(layer)
        layer = kl.Activation("relu")(layer)

        layer = kl.Concatenate()([layer, layer2])
