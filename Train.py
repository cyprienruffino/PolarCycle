...
# Initialize the err of image

# Build the graph to for compute the error of constraint C1
x = tf.placeholder(tf.float32, shape=[1,500,500,4],name='x')
ComputeErrC1 = EvalImageConstraintC1(x)
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(sess.run(self.global_step),nb_epoch):                
                ...
                for ptr in range(0,max_img):
                    ...
                    feed_dict={self.input_A:self.A_input[ptr], self.input_B:self.B_input[ptr], self.lr:curr_lr, self.fake_pool_B:fake_B_temp1,self.fake_pool_A:fake_A_temp1})
                ...
                for i in range(10):
                    fake_B,fake_A = sess.run([self.fake_B,self.fake_A],feed_dict={self.input_A:self.A_input[i], self.input_B:self.B_input[i], self.lr:curr_lr, self.fake_pool_B:fake_B_temp1,self.fake_pool_A:fake_A_temp1})
